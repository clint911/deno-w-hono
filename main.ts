...
animals
    .get("/:id", (c) => {
     ...
     .put(async (c) => {
        const { id } = c.req.param();
        const userInput = await c.req.json<{ owner: string }>();
        const stmt = animalDb.prepare("SELECT * FROM pets WHERE id = ?;");
        const row = stmt.get<Animal>(id);
        if (!row) {
            throw new HTTPException(404, {
...
people.get("/:id", (c) => {
    const { id } = c.req.param();
    const personalDetail = factoryPeople.find((el) => el.id === Number.parseInt(id));
    if (!personalDetail) {
        throw new HTTPException(404, {
            message: "ID not found.",
         });
 ...
 throw new HTTPException(503, {
    message: "Internal Server Error. Unable to delete user in the database.",
    });
  });
